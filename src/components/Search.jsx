...
const Search = () => {
    ...
    useEffect(() => {
        ...
        setRandomHour (Math.floor(Math.random() * 25));
    ...
    return (
        <div className="container">
                                ...
                                <div className="flight-summary-infos">
                                    ...
                                    <div className="flight-summary-time">
                                        ...
                                        <div className="summary-duration">
                                            <Schedule /> {randomHour}sa {randomMinute}dk
                                        </div>
                                    </div>
                                    ...
                                    <div className="segment-detail ">
                                        <div className="segmentInfo">
                                            <div className="segment-airline">
                                                <span className="segment-airline-name">Türk Hava Yolları </span>
                                            </div>
                                            <div>
                                                <span className="flightNumber">Uçuş:  <span>TK2108</span></span>
                                            </div>
                                            <div className="segment-flight">
                                                <div>
                                                    <div className="segment-class">Sınıfı: <span>E</span></div>
                                                </div>
                                                <div className="segment-airports">
                                                    ...
                                                    <div className="segment-airport-destination">
                                                        <div title="Ankara, Esenboğa Havalimanı" className="combineAirportName noneWidth">{maxRandomTime}:00 - {flightDate.return_date.returnDate === 'Tek Yön' ? 'Tek Yön' : dayjs(flightDate.return_date.returnDate).format('DD MMM dddd')} - {returnAirport.return_airport_name.value}</div>
                                                        <div className="segment-airport-terminal"></div>
                                                    </div>
                                                </div>
            ...
            <div className="row">
                <div className="col">
                <h3>Departure Airport : {departureAirport.departure_airport_name.value}</h3>
                <h3>Return Airport : {returnAirport.return_airport_name.value}</h3>
                <h3>Departure Date : {dayjs(flightDate.departure_date.startDate).format('DD MMM YYYY, ddd')}</h3>
                <h3>Return Date : {flightDate.return_date.returnDate === 'Tek Yön' ? 'Tek Yön' : dayjs(flightDate.return_date.returnDate).format('DD MMM YYYY, ddd')}</h3>
                <h3>Passenger Type : {passengerType.passenger_type.person}</h3>
                </div>
            </div>
        </div>
    );
